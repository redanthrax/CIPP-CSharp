...
public static class ListSharePointSites {
    ...
    private static async Task<IResult> Handle(
        ...
        try {
            ...
            var query = new GetSharePointSitesQuery(tenantId, type, pagingParams);
            var result = await mediator.Send(query, cancellationToken);

            return Results.Ok(Response<PagedResponse<SharePointSiteDto>>.SuccessResult(result, "SharePoint sites retrieved successfully"));
        } catch (Exception ex) {

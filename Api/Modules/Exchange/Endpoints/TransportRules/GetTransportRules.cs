...
public static class GetTransportRules {
    ...
    private static async Task<IResult> Handle(
        ...
        try {
            ...
            var query = new GetTransportRulesQuery(tenantId, pagingParams);
            var result = await mediator.Send(query, cancellationToken);

            return Results.Ok(Response<PagedResponse<TransportRuleDto>>.SuccessResult(result, "Transport rules retrieved successfully"));
        } catch (Exception ex) {

...
public static class GetContacts {
    ...
    private static async Task<IResult> Handle(
        ...
        try {
            ...
            var query = new GetContactsQuery(tenantId, pagingParams);
            var result = await mediator.Send(query, cancellationToken);

            return Results.Ok(Response<PagedResponse<ContactDto>>.SuccessResult(result, "Contacts retrieved successfully"));
        } catch (Exception ex) {
